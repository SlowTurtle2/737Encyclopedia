...
function Light({ title, children }: { title: string; children: React.ReactNode }) { return <div className="system-light-card"><strong>{title}</strong><p>{children}</p></div>; }
  ...
  return <main id="main" className="system-course">
    ...
    <div className="wrap course-layout"><article>
      <section className="course-section" id="schematic">
        ...
        <p>Two engine bleed systems feed a common duct divided by an isolation valve. The APU and external pneumatic connection feed the same distribution duct. From there, air can supply the packs, engine starters, wing anti-ice, hydraulic-reservoir and water-tank pressurization, and the nitrogen generation system.</p>
        <Placeholder>Global bleed-air schematic showing both engines, the APU, external air, isolation valve, packs and principal users.</Placeholder>
      ...
      <section className="course-section" id="bleed">
        ...
        <h3>Sources</h3>
        <ul className="study-points"><li><strong>Engines:</strong> the normal inflight source. Low-stage compressor air supplies most conditions; a high-stage valve opens when low-stage pressure is insufficient.</li><li><strong>APU:</strong> supplies both packs on the ground or one pack in flight and provides pneumatic air for engine start.</li><li><strong>External air:</strong> connects to the right side of the manifold and can supply air conditioning or engine starting.</li></ul>
        ...
        <Deep title="Go deeper · External engine start"><p>The ground-air connection is nearer engine No. 2. Starting engine No. 1 first gives the external source the shorter effective supply path for the second start after the manifold configuration changes.</p></Deep>
        ...
        <h4>Pack switches</h4>
        <ul className="study-points"><li><strong>OFF:</strong> closes the related pack valve.</li><li><strong>AUTO:</strong> provides normal pack flow. In flight with flaps up, both engine bleeds ON and both packs selected AUTO, the remaining pack automatically changes to high flow after a pack failure, engine failure or selection of the other pack OFF.</li><li><strong>HIGH:</strong> commands high pack flow for increased ventilation.</li></ul>
        <p>Automatic high flow is inhibited on the ground or with flaps extended to preserve engine performance. With the APU as the sole source, both engine bleeds OFF and both packs AUTO, one remaining pack automatically changes to high flow regardless of flap or air/ground status. Selecting either pack HIGH in this configuration commands APU high flow.</p>
        <h4>Isolation valve switch</h4>
        <ul className="study-points"><li><strong>CLOSE:</strong> commands the isolation valve closed and separates the left and right manifolds.</li><li><strong>AUTO:</strong> closes the valve when both engine bleeds are ON and both pack switches are in AUTO or HIGH. It opens the valve if either engine bleed switch or either pack switch is OFF.</li><li><strong>OPEN:</strong> commands the isolation valve open, joining both manifold sides.</li></ul>
        <Need title="AUTO responds to engine-bleed and pack switches"><p>APU BLEED switch position does not change isolation-valve position in AUTO. The automatic logic responds to the engine bleed and pack switch configuration.</p></Need>

        <h3>Indications and warnings</h3>
        <div className="system-light-grid"><Light title="BLEED / BLEED TRIP OFF">A bleed overtemperature, overpressure or system fault closes the related engine bleed valve and illuminates the light. On the MAX, both BLEED lights can also indicate an incorrect post-takeoff or go-around configuration.</Light><Light title="WING-BODY OVERHEAT">Indicates a bleed-duct leak in the related engine strut, inboard wing leading edge or air-conditioning bay. The left indication also monitors the keel beam and APU bleed duct.</Light><Light title="DUAL BLEED">Indicates a configuration that can allow engine bleed pressure to backpressure the APU. Engine thrust must remain at idle while the light is illuminated.</Light><Light title="DUCT PRESSURE">The two needles show left and right manifold pressure. A difference between them is acceptable if sufficient air remains available for pressurization.</Light></div>
      ...
      <section className="course-section" id="conditioning">
        ...
        <h3>Schematic</h3>
        <p>Bleed air first crosses a heat exchanger, then enters the air-cycle machine for refrigeration. Cold processed air is mixed with bypass air, moisture is removed by the water separator, and the resulting conditioned air enters the mix manifold. Ram-air doors regulate cooling airflow through the heat exchangers.</p>
        <Placeholder>Air-conditioning pack schematic showing the pack valve, heat exchangers, air-cycle machine, bypass path and water separator.</Placeholder>
        <Deep title="Go deeper · Ram-air doors"><p>The doors move fully open on the ground and during slow flight with the flaps not fully retracted. In normal cruise they modulate as required. Deflector doors protect the ram-air inlets from slush before liftoff and after touchdown.</p></Deep>
        ...
        <Placeholder>Conditioned-air distribution and trim-air schematic for the three temperature zones.</Placeholder>
        ...
        <h3>Indications and warnings</h3>
        <div className="system-light-grid"><Light title="PACK / PACK TRIP OFF">Indicates a pack overheat or loss of both primary and standby pack control. The related pack valve closes after an overheat trip.</Light><Light title="ZONE TEMP">Indicates an overheat in the related zone duct or a temperature-control failure. The associated trim-air valve closes for duct overheat.</Light><Light title="EQUIP COOLING SUPPLY / EXHAUST OFF">Indicates loss of airflow caused by failure of the related cooling fan. Selecting ALTN should restore flow and extinguish the light.</Light><Light title="EQUIP SMOKE · MAX">Illuminates with MASTER CAUTION and OVERHEAD when smoke is detected in an equipment-cooling duct.</Light></div>
      ...
      <section className="course-section" id="pressurization">
        ...
        <div className="system-light-grid"><Light title="AUTO FAIL">Indicates loss of DC power, a controller fault, an outflow-valve control fault, or an improperly controlled excessive differential pressure, cabin rate or cabin altitude. The system automatically transfers to the alternate controller.</Light><Light title="ALTN">Indicates that the alternate automatic controller is operating. With AUTO FAIL, it confirms automatic transfer to single-channel operation.</Light><Light title="OFF SCHED DESCENT">The aircraft has begun descending before reaching the selected cruise altitude. The controller schedules the cabin back toward takeoff-field elevation unless FLT ALT is changed.</Light><Light title="MANUAL">Illuminates green when MAN is selected and the crew directly controls outflow-valve position.</Light></div>
