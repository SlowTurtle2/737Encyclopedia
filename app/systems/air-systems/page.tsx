...
function Figure({ src, alt, caption, compact = false }: { src: string; alt: string; caption: string; compact?: boolean }) { return <figure className={`figure air-system-figure${compact ? ' compact' : ''}`}><img src={src} alt={alt} /><figcaption>{caption}</figcaption></figure>; }
function Light({ title, src, alt, children }: { title: string; src?: string; alt?: string; children: React.ReactNode }) { return <div className="system-light-card">{src && <img className="air-system-light" src={src} alt={alt ?? title} />}<strong>{title}</strong><p>{children}</p></div>; }
  ...
  return <main id="main" className="system-course">
    ...
    <div className="wrap course-layout"><article>
      ...
      <section className="course-section" id="bleed">
        ...
        <h3>Command</h3>
        <Figure compact src="/images/air-system-bleed-panel.png" alt="737 bleed-air control panel" caption="Bleed-air panel: pack, isolation-valve, engine-bleed and APU-bleed controls." />
        ...
        <div className="system-light-grid"><Light title="BLEED / BLEED TRIP OFF" src="/images/air-system-bleed-trip-off-light.png" alt="Amber BLEED TRIP OFF light">A bleed overtemperature, overpressure or system fault closes the related engine bleed valve and illuminates the light. On the MAX, both BLEED lights can also indicate an incorrect post-takeoff or go-around configuration.</Light><Light title="WING-BODY OVERHEAT" src="/images/air-system-wing-body-overheat-light.png" alt="Amber WING-BODY OVERHEAT light">Indicates a bleed-duct leak in the related engine strut, inboard wing leading edge or air-conditioning bay. The left indication also monitors the keel beam and APU bleed duct.</Light><Light title="DUAL BLEED" src="/images/air-system-dual-bleed-light.png" alt="Amber DUAL BLEED light">Indicates a configuration that can allow engine bleed pressure to backpressure the APU. Engine thrust must remain at idle while the light is illuminated.</Light><Light title="DUCT PRESSURE">The two needles show left and right manifold pressure. A difference between them is acceptable if sufficient air remains available for pressurization.</Light></div>
      ...
      <section className="course-section" id="conditioning">
        ...
        <h3>Indications and warnings</h3>
        <div className="system-light-grid"><Light title="PACK / PACK TRIP OFF" src="/images/air-system-pack-light.png" alt="Amber PACK light">Indicates a pack overheat or loss of both primary and standby pack control. The related pack valve closes after an overheat trip.</Light><Light title="ZONE TEMP" src="/images/air-system-zone-temp-light.png" alt="Amber ZONE TEMP light">Indicates an overheat in the related zone duct or a temperature-control failure. The associated trim-air valve closes for duct overheat.</Light><Light title="EQUIP COOLING SUPPLY / EXHAUST OFF">Indicates loss of airflow caused by failure of the related cooling fan. Selecting ALTN should restore flow and extinguish the light.</Light><Light title="EQUIP SMOKE · MAX" src="/images/air-system-equip-smoke-light.png" alt="Amber EQUIP SMOKE light">Illuminates with MASTER CAUTION and OVERHEAD when smoke is detected in an equipment-cooling duct.</Light></div>
