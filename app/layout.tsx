...
function Navigation() {
  return <><Link href="/">Home</Link><details className="systems-menu"><summary>Systems</summary><div>{chapters.map(name => <Link key={name} href={'/systems/' + slug(name)}>{name}</Link>)}</div></details><Link href="/fmc">FMC</Link><Link href="/max-differences">MAX differences</Link><Link href="/descent-management">Descent Management</Link><Link href="/glossary">Glossary</Link><Link href="/academy">Quiz</Link><Link href="/weird-things">Weird things</Link></>;
}
