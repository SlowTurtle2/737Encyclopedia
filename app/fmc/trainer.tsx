...
const pageCounts: Partial<Record<PageName, number>> = { 'POS INIT': 3, 'RTE': 2, 'LEGS': 3, 'PERF INIT': 2, 'TAKEOFF REF': 2, 'FIX INFO': 2 };
// Takeoff N1 limit falls with each derate (arbitrary training values).
const N1_BY_MODE: Record<'TO' | 'TO-1' | 'TO-2', string> = { 'TO': '94.6/94.6', 'TO-1': '91.9/91.9', 'TO-2': '88.7/88.7' };
...
// A DEP/ARR INDEX style row: centred label above, then a left prompt, a
// centred airport code and a right prompt (LSK left / RSK right).
function DaRow({ label, left, mid, right }: { label?: string; left?: string; mid?: string; right?: string }) {
  return <div className="cdu-line da-row"><small className="da-label">{label || ' '}</small><div className="da-cells"><span className="da-l">{left || ''}</span><span className="da-m">{mid || ''}</span><span className="da-r">{right || ''}</span></div></div>;
}
...
export function FmcTrainer() {
  ...
  if (page === 'LEGS') { leftActions[5] = () => setMessage('RTE 2 NOT AVAILABLE'); rightActions[5] = execute; }
  ...
  if (page === 'DEPARTURES') { leftActions[0] = () => { setSid((s) => (s === 'LACRE3' ? '' : 'LACRE3')); setModified(true); }; leftActions[1] = () => { setSidTrans((t) => (t === 'HUMPP' ? '' : 'HUMPP')); setModified(true); }; leftActions[2] = () => { setSidTrans((t) => (t === 'ORTIN' ? '' : 'ORTIN')); setModified(true); }; leftActions[3] = () => { setSidTrans((t) => (t === 'VAMPS' ? '' : 'VAMPS')); setModified(true); }; rightActions[0] = () => { setRoute((r) => ({ ...r, runway: '13R' })); setModified(true); }; rightActions[1] = () => { setRoute((r) => ({ ...r, runway: '31L' })); setModified(true); }; leftActions[5] = () => { setSid(''); setSidTrans(''); }; rightActions[5] = () => { if (sid) { const via = sidTrans ? `${sid}.${sidTrans}` : sid; setAirways((a) => { const n = a.slice(); n[0] = via; return n; }); if (sidTrans) setRoutePoints((p) => { const n = p.slice(); n[0] = sidTrans; return n; }); setModified(true); } go('RTE'); }; }
  ...
  function displayedLineValues() {
    ...
    if (page === 'N1 LIMIT') { left[0] = `${extra.selTemp}/${extra.oat}`; right[0] = N1_BY_MODE[takeoffMode]; left[2] = 'TO-1'; right[2] = 'CLB-1'; left[3] = 'TO-2'; right[3] = 'CLB-2'; }
    if (page === 'TAKEOFF REF' && pageNumber === 1) { left[0] = extra.flaps; right[0] = extra.v1; left[1] = N1_BY_MODE[takeoffMode]; right[1] = extra.vr; left[2] = extra.cg || '22.5'; right[2] = extra.v2; left[4] = route.runway; right[4] = 'QRH OFF'; }
  ...
  function ScreenContent() {
    ...
    if (page === 'N1 LIMIT') return <><Pair leftLabel="SEL/OAT" left={`${extra.selTemp || '---'}/${extra.oat || '---'}°C`} rightLabel={`${takeoffMode} N1`} right={N1_BY_MODE[takeoffMode]}/><Pair left={`<TO${takeoffMode === 'TO' ? '  <ACT>' : ''}`} right={`${climbMode === 'CLB' ? '<ACT>  ' : ''}CLB>`}/><Pair leftLabel="XX% DERATE" left={`<TO-1${takeoffMode === 'TO-1' ? '  <ACT>' : ''}`} right={`${climbMode === 'CLB-1' ? '<ACT>  ' : ''}CLB-1>`}/><Pair leftLabel="XX% DERATE" left={`<TO-2${takeoffMode === 'TO-2' ? '  <ACT>' : ''}`} right={`${climbMode === 'CLB-2' ? '<ACT>  ' : ''}CLB-2>`}/><Pair/><NavLine left="<PERF INIT" right="TAKEOFF>"/></>;
    if (page === 'LEGS' && pageNumber === 2) return <><Pair leftLabel="065°  22NM" left="ELN" right="250/12000"/><Pair leftLabel="078°  15NM" left="EPH" right=".740/FL230"/><Pair leftLabel="081°  19NM" left="MWH" right=".740/FL250"/><Pair leftLabel="095°  12NM" left="RUBEE" right="280/FL250"/><Pair/><NavLine left="<RTE 2 LEGS" right="ACTIVATE>"/></>;
    if (page === 'LEGS' && pageNumber === 3) return <><Pair leftLabel="110°  18NM" left="GLASR" right="250/10000"/><Pair leftLabel="123°  9NM" left={activeRoute.destination || 'KMWH'} right="---/ -----"/><Pair leftLabel="-------- END OF RTE --------"/><Pair/><Pair/><NavLine left="<RTE 2 LEGS" right="ACTIVATE>"/></>;
    if (page === 'LEGS') return <><Pair leftLabel="129°HDG  3.1NM" left="(1000)" right="166/ 1000A"/><Pair leftLabel="131°  4.2NM" left="LACRE" right="214/ 2500"/><Pair leftLabel="128°  1.8NM" left="LACO1" rightLabel="GP X.XX" right="250/ 4690"/><Pair leftLabel="70°  6.9NM" left="VAMPS" right="250/ 8000A"/><Pair leftLabel="-RNP/ACTUAL-------------" left="1.00/0.21NM"/><NavLine left="<RTE 2 LEGS" right="ACTIVATE>"/></>;
    if (page === 'DEP ARR') return <><DaRow label="RTE 1 (ACT)" left="<DEP" mid={activeRoute.origin || route.origin || '----'} right="ARR>"/><DaRow mid={activeRoute.destination || route.destination || '----'} right="ARR>"/><DaRow label="---------- RTE 2 ----------" left="<DEP" mid={activeRoute.destination || route.destination || '----'} right="ARR>"/><DaRow mid={activeRoute.origin || route.origin || '----'} right="ARR>"/><Pair/><div className="cdu-line da-navrow"><div><small>DEP</small><strong>&lt;----</strong></div><div><small>ARR</small><strong>----&gt;</strong></div></div></>;
    ...
    if (page === 'TAKEOFF REF' && pageNumber === 1) return <><Pair leftLabel="FLAPS" left={extra.flaps || '□□'} rightLabel="V1" right={extra.v1 || '---'}/><Pair leftLabel={`${takeoffMode} N1`} left={`${N1_BY_MODE[takeoffMode]}%`} rightLabel="VR" right={extra.vr || '---'}/><Pair leftLabel="CG   TRIM" left={`${extra.cg || '22.5%'} / 5.25`} rightLabel="V2" right={extra.v2 || '---'}/><Pair/><Pair leftLabel="RUNWAY" left={route.runway || '----------'} rightLabel="SELECT" right="QRH OFF>"/><NavLine left="<PERF INIT"/></>;
